    ...
    if (opts.renderData) { // 将 cache 信息重新渲染
      ...
      $.ajax({
        ...
        success: function(res){
          ...
          if(res[response.statusName] != response.statusCode){
            that.errorView(
              ...
              ('The returned data does not conform to the specifications. The correct success status code should be："'+ response.statusName +'": '+ response.statusCode)
            );
          } else {
        ...
        error: function(e, msg){
          that.errorView('Requset Error：'+ msg);
