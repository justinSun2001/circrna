...
if (isset($_GET['BloodCircR_ID']) && isset($_GET['page']) && isset($_GET['limit'])) {
    ...
    // 使用预处理语句
    $countSql = "SELECT COUNT(*) AS total FROM rbp_info WHERE BloodCircR_ID = ?";
    ...
    // 使用预处理语句
    $sql = "SELECT * FROM rbp_info WHERE BloodCircR_ID = ? ORDER BY `$sortField` $sortOrder LIMIT ? OFFSET ?";
