...
function writeDB($datasetname) {
	...
	$file1 = '/data2/njucm1/BloodCircleR/Expression/cache/differential_expression_3.svg';  
	$file2 = '/data2/njucm1/BloodCircleR/Expression/cache/differential_expression_3.csv';
    // 检查文件是否存在
    ...
    $sql = "CREATE TABLE IF NOT EXISTS differential_expression_3 (
    ...
    $insert_sql = "INSERT INTO differential_expression_3 (Dataset, image1, data1) VALUES (?, ?, ?)
...
function fetchImagesAndCSVByDatasetName($datasetname) {
    global $conn; // 引入全局的$conn变量到函数作用域内
    $sql = "SELECT image1, data1 FROM differential_expression_3 WHERE Dataset = ?";
...
$param5 = $_GET['FoldChange'];
 ...
 $rScriptPath = $rScriptDir."differential_expression_3.R"; // R脚本的路径
   
   $command = "Rscript {$rScriptPath}  '{$param2}' '{$param3}' '{$param4}' '{$param5}' 2>&1";
