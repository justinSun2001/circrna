...
function checkDatasetExists($datasetname) {
    ...
    // 创建表的SQL语句
    $sqlCreateTable = "CREATE TABLE IF NOT EXISTS sample_expression (
        `Dataset` VARCHAR(255),
        `image1` LONGBLOB
    )";
    ...
    $sql = "SELECT COUNT(*) FROM sample_expression WHERE Dataset = ?";
...
function writeDB($datasetname, $param1) {
    ...
    $file1 = $param1 . 'sample_expression.png';
    ...
    $sql = "CREATE TABLE IF NOT EXISTS sample_expression (
        `Dataset` VARCHAR(255),
        `image1` LONGBLOB,
    )";
    $conn->query($sql);

    $insert_sql = "INSERT INTO sample_expression (Dataset, image1) VALUES (?, ?)";
...
function fetchImagesByDatasetName($datasetname) {
    global $conn; // 引入全局的$conn变量到函数作用域内
    $sql = "SELECT image1 FROM sample_expression WHERE Dataset = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("s", $datasetname);
    $stmt->execute();
    $result = $stmt->get_result();
...
$rScriptPath = $rScriptDir ."sample_expression.R"; // R脚本的路径
