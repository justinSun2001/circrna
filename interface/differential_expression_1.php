...
function writeDB($datasetname) {
	...
	$file1 = '/data2/njucm1/BloodCircleR/Expression/cache/differential_expression_1.svg';  
	$file2 = '/data2/njucm1/BloodCircleR/Expression/cache/differential_expression_1.csv';
    // 检查文件是否存在
    ...
    $sql = "CREATE TABLE IF NOT EXISTS differential_expression_1 (
    ...
    $insert_sql = "INSERT INTO differential_expression_1 (Dataset, image1, data1) VALUES (?, ?, ?)
...
function fetchImagesAndCSVByDatasetName($datasetname) {
    global $conn; // 引入全局的$conn变量到函数作用域内
    $sql = "SELECT image1, data1 FROM differential_expression_1 WHERE Dataset = ?";
...
$datasetname = isset($_POST['datasetname']) ? $_POST['datasetname'] : (isset($_GET['datasetname']) ? $_GET['datasetname'] : '');
    ...
    $param2 = $datasetname;
    // 获取其他参数
    $param3 = isset($_GET['compare']) ? $_GET['compare'] : '';
    $param4 = isset($_GET['level']) ? $_GET['level'] : '';
    $param5 = isset($_GET['pvalue']) ? $_GET['pvalue'] : '';
    $param6 = isset($_GET['FoldChange']) ? $_GET['FoldChange'] : '';
    $param7 = isset($_GET['detectR']) ? $_GET['detectR'] : '';
 
    // 构建R脚本执行命令，参数通过命令行传给R脚本
 
   $rScriptPath = $rScriptDir . "differential_expression_1.R"; // R脚本的路径
