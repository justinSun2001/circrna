...
if (isset($_GET['BloodCircR_ID']) && isset($_GET['page']) && isset($_GET['limit'])) {
    ...
    // 使用预处理语句
    $countSql =  "SELECT COUNT(*) AS total FROM expression_means2 WHERE host_gene IN (SELECT host_gene FROM circRNA_info WHERE BloodCircR_ID = ?)";
    ...
    // 使用预处理语句
    $sql = "SELECT disease, TPM FROM expression_means2 WHERE host_gene IN (SELECT host_gene FROM circRNA_info WHERE BloodCircR_ID = ?) ORDER BY `$sortField` $sortOrder LIMIT ?, ?";
